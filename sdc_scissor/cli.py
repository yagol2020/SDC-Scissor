...
def my_extract_features_fun(road_loc_x_y):
    """
    自定义的提取特征的函数
    """
    test_validator = NoIntersectionValidator(SimpleTestValidator())
    ...
    my_test = Test(test_id=1, road_points=road_loc_x_y, test_outcome=None, test_duration=None)
    test_validator.validate(my_test)
    road_features = feature_extractor.extract_features(my_test)
    return road_features


def extract_features_func_to_resut(tests: Path, segmentation: str = "angle-based"):
    """
    提取特征的函数
    """
    ...
    return road_features_lst


def extract_features_func(tests: Path, segmentation: str = "angle-based") -> None:
    """
    ...
    """
    road_features_lst = extract_features_func_to_resut(tests, segmentation)
...
def extract_features(tests: Path, segmentation: str) -> None:
    extract_features_func(tests, segmentation)
